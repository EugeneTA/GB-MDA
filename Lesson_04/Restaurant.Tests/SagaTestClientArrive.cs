...
{
    public class SagaTestClientArrive
    {
        [Test]
        public async Task Saga_Booking_Request_Confirmed_And_Client_Arrived()
        {
            ...
            Assert.That(await sagaHarness.Consumed.Any<IClientArrived>(), Is.True);
            Assert.That(await sagaHarness.Consumed.Any<IClientArriveExpired>(), Is.False);
        }
    }
}
